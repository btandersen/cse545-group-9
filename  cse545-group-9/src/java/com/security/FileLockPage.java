            ...
            try
            {
                ...
                if (userRs.next())
                {
                    ...
                    if (userIsManager)
                    {
                        ...
                        shareQuery = "SELECT A.did, A.title, A.auth, A.dept, A.ouid, A.filename FROM Docs A, Shared B WHERE B.sdid=A.did AND B.suid=" + uid + " AND NOT EXISTS (SELECT * FROM Locked L WHERE A.did=L.ldid)";;
                    }
